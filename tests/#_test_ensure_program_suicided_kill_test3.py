...
def test_ensure_program_suicided_kill3():
    """ensure_file_found.py 창 닫기 테스트"""
    ensure_printed("🔍 ensure_file_found.py 창 닫기 테스트", print_color="blue")
    ...
    try:
        ...
        # ensure_file_found.py 창 찾기
        target_windows = [(hwnd, title) for hwnd, title in all_windows 
                         if 'ensure_file_found.py' in title]
        
        ensure_printed(f"📊 ensure_file_found.py 창 개수: {len(target_windows)}", print_color="blue")
    ...
    try:
        ...
        # ensure_file_found.py 파일 경로
        target_file = "pkg_py/functions_split/ensure_file_found.py"
    ...
    try:
        ...
        ensure_printed(f"📊 호출 후 ensure_file_found.py 창 개수: {len(target_windows_after)}", print_color="blue")
...
if __name__ == "__main__":
    test_ensure_program_suicided_kill3() 