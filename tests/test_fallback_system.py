...
def test_fallback_system():
    """Fallback 시스템 테스트"""
    ensure_printed("🧪 Fallback 시스템 테스트 시작", print_color="cyan")
    ...
    for i, url in enumerate(test_urls, 1):
        ensure_printed(f"🔍 테스트 {i}: {url}", print_color="yellow")
        ensure_printed("=" * 50, print_color="white")
        
        # 1. 메타데이터 추출 테스트
        ensure_printed("📊 1단계: 메타데이터 추출 테스트", print_color="cyan")
        try:
            info, title, clip_id, ext_result = get_youtube_video_metadata(yt_dlp, url)
            if info and title and clip_id:
                ensure_printed(f"✅ 메타데이터 성공: {title} ({clip_id})", print_color="green")
            else:
                ensure_printed("❌ 메타데이터 실패", print_color="red")
                continue
        except Exception as e:
            ensure_printed(f"❌ 메타데이터 예외: {str(e)[:100]}", print_color="red")
            continue
        
        # 2. 다운로드 테스트
        ensure_printed("📥 2단계: 다운로드 테스트", print_color="cyan")
        try:
            result = download_youtube_video_via_yt_dlp_v2(D_FFMPEG_LOCATION, D_PK_WORKING, ext, url)
            if result:
                ensure_printed("✅ 다운로드 성공", print_color="green")
            else:
                ensure_printed("❌ 다운로드 실패", print_color="red")
        except Exception as e:
            ensure_printed(f"❌ 다운로드 예외: {str(e)[:100]}", print_color="red")
...
def test_basic_vs_fallback():
    ...
    ensure_printed("🧪 기본 옵션 vs Fallback 옵션 비교 테스트", print_color="cyan")
    ...
    # 1. 기본 옵션 테스트
    ensure_printed("🔍 기본 옵션 테스트", print_color="yellow")
    try:
        with yt_dlp.YoutubeDL(basic_opts) as ydl:
            info = ydl.extract_info(test_url, download=False)
            if info:
                ensure_printed("✅ 기본 옵션: 성공", print_color="green")
            else:
                ensure_printed("❌ 기본 옵션: 실패", print_color="red")
    except Exception as e:
        ensure_printed(f"❌ 기본 옵션: {str(e)[:100]}", print_color="red")
    
    # 2. Fallback 옵션 테스트
    ensure_printed("🔄 Fallback 옵션 테스트", print_color="yellow")
    try:
        with yt_dlp.YoutubeDL(fallback_opts) as ydl:
            info = ydl.extract_info(test_url, download=False)
            if info:
                ensure_printed("✅ Fallback 옵션: 성공", print_color="green")
            else:
                ensure_printed("❌ Fallback 옵션: 실패", print_color="red")
    except Exception as e:
        ensure_printed(f"❌ Fallback 옵션: {str(e)[:100]}", print_color="red")


def test_error_pattern_detection():
    """오류 패턴 감지 테스트"""
    ensure_printed("🧪 오류 패턴 감지 테스트", print_color="cyan")
...
def main():
    ...
    ensure_printed("=" * 60, print_color="white")
    ensure_printed("🏁 모든 Fallback 테스트 완료", print_color="cyan")
