def test_wsl_제어():
    ...
    if not state_wsl_installed:
        debug_state_for_py_data_type(pk_stamp='%%%FOO%%%-2', data_working=pk_wsl_cmd_map_dict)
        ensure_printed("wsl installed", print_color='red')
        sys.exit(0)
    else:
        if LTA:
            debug_state_for_py_data_type(pk_stamp='%%%FOO%%%-3', data_working=pk_wsl_cmd_map_dict)
