...
def pk_ensure_pnxs_renamed_from_pattern_once_to_pattern_new(src, pattern, mode, pattern_new, with_walking):
    import re
    ...
    if mode == "f":
        ...
        pk_ensure_printed(str_working=rf'''"return"  {'%%%FOO%%%' if LTA else ''}''', print_color='red')
        return
    pk_ensure_printed(f'''len(pnxs)={len(pnxs)}  {'%%%FOO%%%' if LTA else ''}''')
    ...
    pk_ensure_printed(str_working=rf'''pnxs_reg_checked="{pnxs_reg_checked}"  {'%%%FOO%%%' if LTA else ''}''')
    ...
    for item in pnxs_reg_checked:
        ...
        # logging
        pk_ensure_printed(str_working=rf'''item_nx_new="{item_nx_new}"  {'%%%FOO%%%' if LTA else ''}''')
    ...
    # pk_ensure_printed(str_working=rf'''len(pnxs_and_pnxs_new)="{len(pnxs_and_pnxs_new)}"  {'%%%FOO%%%' if LTA else ''}''')
