...
def ensure_pnxs_renamed_from_pattern_once_to_pattern_new(src, pattern, mode, pattern_new, with_walking):
    import re
    ...
    if mode == "f":
        ...
        return
    ensure_printed(f'''len(pnxs)={len(pnxs)}  {'%%%FOO%%%' if LTA else ''}''')
    ...
    ensure_printed(str_working=rf'''pnxs_reg_checked="{pnxs_reg_checked}"  {'%%%FOO%%%' if LTA else ''}''')
    ...
    for item in pnxs_reg_checked:
        ...
        # logging
        ensure_printed(str_working=rf'''item_nx_new="{item_nx_new}"  {'%%%FOO%%%' if LTA else ''}''')
