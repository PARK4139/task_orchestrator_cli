...
def pk_ensure_pnxs_renamed_from_pattern_twice_to_pattern_new(pnx, pattern, mode, with_walking, pattern_new="_"):
    import re
    ...
    pk_ensure_printed(f'''len(pnxs_reg_checked)={len(pnxs_reg_checked)}  {'%%%FOO%%%' if LTA else ''}''')
    ...
    # 확인
    pk_ensure_printed(str_working=rf'''len(pnxs_and_pnxs_new)="{len(pnxs_and_pnxs_new)}" 바꿀 대상  {'%%%FOO%%%' if LTA else ''}''')
