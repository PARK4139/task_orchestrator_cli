...
def pk_ensure_pnxs_renamed_from_keywords_to_keyword_new_at_d(d, mode, with_walking, debug_mode=True):
    import inspect
    import re
    ...
    if mode == "f":
        ...
        pk_ensure_printed(str_working=rf'''mode={mode}  {'%%%FOO%%%' if LTA else ''}''')
        return
    ...
    for item in pnxs:
        ...
        # pk_ensure_printed(str_working=rf'''item_pnx="{item_pnx}"  {'%%%FOO%%%' if LTA else ''}''')
        # pk_ensure_printed(str_working=rf'''item_pnx_new="{item_pnx_new}"  {'%%%FOO%%%' if LTA else ''}''')
        if item_pnx != item_pnx_new:  # item_pnx_와 item_pnx_new가 다르면 추가
            pnxs_and_pnxs_new.append([item_pnx, item_pnx_new])

    # 확인
    pk_ensure_printed(f'''pnxs_and_pnxs_new={pnxs_and_pnxs_new}  {'%%%FOO%%%' if LTA else ''}''')
    pk_ensure_printed(f'''len(pnxs_and_pnxs_new)={len(pnxs_and_pnxs_new)} 바꿀 대상  {'%%%FOO%%%' if LTA else ''}''')
