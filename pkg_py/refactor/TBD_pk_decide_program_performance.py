...
@staticmethod
def print_pk_ensure_seconds_measured_performance_nth_for_비교군1(function):
    ...
    def wrapper(**kwargs):  # **kwargs, keyword argument, dictionary 로 parameter 를 받음. named parameter / positional parameter 를 받을 사용가능?
        ...
        # def wrapper():
        pk_ensure_printed(str_working=rf'''{PK_UNDERLINE}{func_n}() %%%FOO%%%''', print_color='blue')
        pk_ensure_printed(str_working=rf'''test_loop_limit="{test_loop_limit}" %%%FOO%%%''', print_color="blue")
        pk_ensure_printed(str_working=rf'''is_first_test_lap="{is_first_test_lap}" %%%FOO%%%''', print_color="blue")
        ...
        for i in range(0, test_loop_limit):
            pk_ensure_printed(str_working=rf'''{PK_UNDERLINE}{i} 번째 테스트 %%%FOO%%%''', print_color="blue")
            time_s = time.time()
            try:
                ...
                pk_ensure_printed(str_working=f'{PK_UNDERLINE}예외발생 s\n\n', print_color="blue")
                pk_ensure_printed(f'''{traceback.format_exc()} %%%FOO%%%''', print_color='red')
                pk_ensure_printed(str_working=f'{PK_UNDERLINE}예외발생 e\n\n', print_color="blue")
                ...
                # pk_ensure_printed(ment=ment, print_color='blue')
                # hold_console()  # cmd /k
                ipdb.set_trace()  # 디버깅을 시작할 지점 # 파이썬 환경이 그대로 이어짐

        if len(seconds) == test_loop_limit:
            pk_ensure_printed(f'''"{UNDERLINE}시간성능측정 테스트 결과 보고"''', print_color="blue")
            pk_ensure_printed(str_working=rf'''test_loop_limit="{test_loop_limit}" %%%FOO%%%''', print_color="blue")
            pk_ensure_printed(str_working=rf'''seconds={seconds} %%%FOO%%%''', print_color="blue")
            seconds_average = sum(seconds) / len(seconds)
            pk_ensure_printed(str_working=rf'''seconds_average="{seconds_average}" %%%FOO%%%''', print_color="blue")
            global 비교군1_결과
...
@staticmethod
def pk_ensure_seconds_measured_performance_nth_for_비교군2(function):
    """시간성능 측정용 코드 as 데코레이터"""
    func_n = inspect.currentframe().f_code.co_name
    ...
    def wrapper(**kwargs):  # **kwargs, keyword argument, dictionary 로 parameter 를 받음. named parameter / positional parameter 를 받을 사용가능?
        ...
        pk_ensure_printed(str_working=rf'''{PK_UNDERLINE}{func_n}() %%%FOO%%%''', print_color='blue')
        pk_ensure_printed(str_working=rf'''test_loop_limit="{test_loop_limit}" %%%FOO%%%''', print_color="blue")
        pk_ensure_printed(str_working=rf'''StateManageis_first_test_lap="{is_first_test_lap}" %%%FOO%%%''', print_color="blue")
...
@print_pk_ensure_seconds_measured_performance_nth_for_비교군1
def run_비교군1():
    if not LTA:
        return
...
@pk_ensure_seconds_measured_performance_nth_for_비교군2
def run_비교군2():
    if not LTA:
        return
...
def print_report_compared_functions_exec_time(function1, function2):
    function1()
    function2()
    pk_ensure_printed(str_working=rf'''{PK_UNDERLINE}비교군 코드 대조 %%%FOO%%%''', print_color='blue')
    pk_ensure_printed(str_working=rf'''비교군1_결과="{비교군1_결과}" %%%FOO%%%''', print_color="blue")
    pk_ensure_printed(str_working=rf'''비교군2_결과="{비교군2_결과}" %%%FOO%%%''', print_color="blue")
    # results = [
    ...
    # pk_ensure_printed(string = rf'''slowest_time="{slowest_time}" %%%FOO%%%''', print_color="blue")
    # pk_ensure_printed(string = rf'''fastest_time="{fastest_time}" %%%FOO%%%''', print_color="blue")
    # pk_ensure_printed(string = rf'''fastest_time_index="{fastest_time_index}" %%%FOO%%%''', print_color="blue")
    results = {
        "비교군1_결과": 비교군1_결과['seconds_average'],
        "비교군2_결과": 비교군2_결과['seconds_average'],
    }
    fastest = min(results, key=results.get)
    pk_ensure_printed(str_working=rf'''fastest="{fastest}" %%%FOO%%%''')
    pk_ensure_printed(str_working=rf'''results[fastest]="{results[fastest]}" %%%FOO%%%''')
    pk_ensure_printed(f'가장 빠른 시간은 "{fastest}" 입니다', print_color="blue")
...
def main():
    ...
    # pk_ensure_pinged(ip=ip_private_mac)
    ...
    # send_pkg_to_house_to_pk_system_pk_ensure_release_server_ran() # todo
    ...
    # send_pkg_to_house_to_pk_system_pk_ensure_release_server_ran()
...
if __name__ == '__main__':
    try:
        ...
        print_function_run_pk_ensure_seconds_measured_via_timeit(function=main, repeat=1)
...
class DataStructureUtil:
    ...
    def is_two_lists_equal(list1, list2):
        ...
        func_n = inspect.currentframe().f_code.co_name
        pk_ensure_printed(str_working=rf'''{PK_UNDERLINE}{func_n}() %%%FOO%%%''')
...
class PkProgramPerformanceOptimizingUtil:
    ...
    def generate_mp3_file_for_time_performance():  # time performance : 9028 초 /60 /60  =  2.5 시간
        ...
        for HH in range(24, 0, -1):
            for mm in range(0, 60):
                pk_ensure_printed(f'{int(HH)}시')
                pk_ensure_printed(f'{int(mm)}분 입니다')
    ...
    def gen_dictionary_for_monitor_pnx_edited_and_back_up(directory_pnx):
        ...
        pk_ensure_printed("딕셔너리 초기화용 코드 시작")
        ...
        pk_ensure_printed("딕셔너리 초기화용 코드 종료")
        # ipdb.set_trace()
...
#         # pk_ensure_printed(f"{str(request.url)} 로 라우팅 시도 중...")
#         pass
#
#     @staticmethod
#     async def preprocess_before_response_return(request, response):
#         # pk_ensure_printed(f"{str(request.url)} 로 라우팅 되었습니다")
#         pass
#
#     @staticmethod
...
#                     pk_ensure_str_writen_to_f(pnx=json_file, txt_str="[]\n")  # 이러한 형태로 객체를 받을 수 있도록 작성해 두어야 받을 수 있음.
#                 else:
#                     if not os.path.isfile(json_file):
#                         with open(json_file, "w", encoding='utf-8') as f:
#                             # json.dump(objects, f, pk_ensure_ascii=False)  # pk_ensure_ascii=False 는 encoding 을 그대로 유지하는 것 같다. ascii 로 변환하는게 안전할 지도 모르겠다.
#                             json.dump(objects, f)  # pk_ensure_ascii=False 는 encoding 을 그대로 유지하는 것 같다. ascii 로 변환하는게 안전할 지도 모르겠다.
#                     else:
...
#             # pk_ensure_printed("db 파일 존재 검사 시도")
...
#         # pk_ensure_printed(f"{inspect.currentframe().f_code.co_name}()")
#         try:
#             key = str(key)
#             db_pnx = DB_YAML
#             try:
...
#             with open(db_pnx, 'r', encoding='utf-8') as f:
#                 db = toml.load(f)
#                 # pk_ensure_printed("DB 업데이트 전 ")
#                 # pk_ensure_printed(context=str(db))
#
#             # 데이터 업데이트
#             if key in db:
#                 db[key] = value
