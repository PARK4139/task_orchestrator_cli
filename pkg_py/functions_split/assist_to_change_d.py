...
from pkg_py.functions_split.get_colorful_str_working_with_stamp_enviromnet import get_colorful_str_working_with_stamp_enviromnet
from pkg_py.functions_split.input_validated import input_validated
from pkg_py.functions_split.ensure_printed import ensure_printed
from pkg_py.functions_split.save_d_to_f import save_d_to_f
from pkg_py.system_object.local_test_activate import LTA
from pkg_py.system_object.map_massages import PkMessages2025
from pkg_py.system_object.directories import D_PKG_PY, D_DESKTOP, D_PKG_TXT, D_PK_WORKING, D_DOWNLOADS, D_J_DRIVE, D_I_DRIVE, D_H_DRIVE, D_G_DRIVE, D_F_DRIVE, D_D_DRIVE, D_C_DRIVE
    ...
    while 1:
        ...
        if len(pk_sys_argv) >= 2:
            if not str.isdigit(pk_sys_argv[1]):
                ensure_printed(f'''{STAMP_TRY_GUIDE} cd {{index}} , index is not str ' {'%%%FOO%%%' if LTA else ''}''',
                         print_color='red')
                continue
            if str.isdigit(pk_sys_argv[1]):
                if int(pk_sys_argv[1]) >= len(d_working_with_idx_dict):
                    ensure_printed(f'''{STAMP_TRY_GUIDE} cd {{index}} , out of index' {'%%%FOO%%%' if LTA else ''}''',
                             print_color='red')
                    continue

        if LTA:
            ensure_printed(f'''pk_cmd_argv={pk_sys_argv} {'%%%FOO%%%' if LTA else ''}''')
            ensure_printed(f'''len(pk_cmd_argv)={len(pk_sys_argv)} {'%%%FOO%%%' if LTA else ''}''')
        ...
        if len(pk_sys_argv) == 2:
            ...
            if str.isdigit(nx_by_user_input):
                if LTA:
                    ensure_printed(f'''인자가 숫자 입니다. {'%%%FOO%%%' if LTA else ''}''')
                ...
                if LTA:
                    ...
                    ensure_printed(f'''f_cd_txt_list={f_cd_txt_list} {'%%%FOO%%%' if LTA else ''}''')
                pk_cd(sys_argv=sys.argv)
                continue
            else:
                if LTA:
                    ensure_printed(f'''인자가 숫자가 아닙니다. {'%%%FOO%%%' if LTA else ''}''')
                continue

        elif len(sys.argv) < 2:
            if LTA:
                ensure_printed(f'인자가 {len(pk_sys_argv)}개 입력되었습니다 {'%%%FOO%%%' if LTA else ''}')
