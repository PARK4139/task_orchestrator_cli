                ...
                for color_text in colors:
                    if color_bg != color_text:
                        ...
                        for setting_key, setting_value in get_display_info().items():
                            pass
                            # ensure_printed(f'setting_key: {setting_key}  ,setting_value: {setting_value}  ')
                        # ensure_printed(f"color {color_bg}{color_text}")
                        for i in range(0, 32):
                            ensure_printed('', print_color='blue')
                        to_right_nbsp = ''
                        for i in range(0, 150):
                            to_right_nbsp = to_right_nbsp + ' '
                        ensure_printed(f"{to_right_nbsp}color {color_bg}{color_text}", print_color='blue')
                        for i in range(0, 32):
                            ensure_printed('', print_color='blue')
