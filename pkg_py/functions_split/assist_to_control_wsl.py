...
def assist_to_control_wsl(**pk_config):
    ...
    class _PkWslController:
        ...
        def _excute_wsl_cmd(self, idx_by_user_input, sys_arg2=None):
            ...
            if exec_cmd is None:
                ensure_printed(f"Unknown command mapping for '{user_cmd_key}'", print_color='red')
                return

            if LTA:
                ensure_printed(f"Trying to run: {exec_cmd}")
            ...
            try:
                ...
                if "-d" in exec_cmd:
                    ...
                    while 1:
                        state_after_list = get_wsl_distro_info_std_list()
                        if LTA:
                            ensure_printed(f'''state_before_list={state_before_list} {'%%%FOO%%%' if LTA else ''}''')
                            ensure_printed(f'''state_after_list={state_after_list} {'%%%FOO%%%' if LTA else ''}''')
                        if state_before_list != state_after_list:
                            print_wsl_distro_info_std_list()
                            break
    ...
    while 1:
        ...
        if is_office_pc():
            if cnt_loop == 1:
                cnt_loop = cnt_loop + 1
                # pk_sys_argv = ['wsl', '1']
                # ensure_slept(seconds = 4) # wsl 실행시간 대기
            if cnt_loop == 2:
                cnt_loop = cnt_loop + 1
                pk_sys_argv = ['wsl', '3']
                pk_seconds_limit = 5
