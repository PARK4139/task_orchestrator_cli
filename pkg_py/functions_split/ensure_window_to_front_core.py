        ...
        def enum_windows_callback(hwnd, lparam):
            if win32gui.IsWindowVisible(hwnd):
                ...
                if window_title_seg.lower() in current_window_title.lower():
                    ...
                    ensure_slept(seconds=0.2)  # 창 복원 후 잠시 대기
                    # SetForegroundWindow를 재시도
                    try:
                        win32gui.SetForegroundWindow(hwnd)  # 창을 제일 앞으로 가져옴
                    except Exception as e:
                        # ensure_printed(f'''f"SetForegroundWindow 호출 재시도 Error: {str(e)}""''')
                        ensure_slept(seconds=0.2)  # 잠시 대기 후 재시도
                        # ensure_printed(f'''f"win32gui.SetForegroundWindow(hwnd) 호출 재시도{str(e)}"" ''', print_color='red')
