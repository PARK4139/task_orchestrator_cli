...
def merge_video_and_sound(f_v, f_a):
    ...
    ensure_printed(f'다운로드 d 생성')
    d_nx_list = ["storage"]
    for d_nx in d_nx_list:
        if not is_d(rf'./{d_nx}'):
            os.makedirs(rf'{d_nx}')

    ensure_printed(rf'merge voiceless video and audio only video ')  # yotube 에서 고해상도 음성 없는 영상과 음성을 받아 하나의 영상으로 merge
    dst = D_VIDEOES_MERGED
    paths = [os.path.abspath(dst), os.path.basename(f_v)]
    f_va = os.path.join(*paths)
    ensure_printed(rf'f_v : {f_v}', print_color='blue')
    ensure_printed(rf'f_a : {f_a}', print_color='blue')
    ensure_printed(rf'f_va : {f_va}', print_color='blue')

    ensure_printed(f'ffmpeg.exe 위치 설정')
    ...
    ensure_printed(f' f머지 시도')
    try:
        ensure_printed(rf'echo y | "{f_ffmpeg_exe}" -i "{f_v}" -i "{f_a}" -c copy "{f_va}"', print_color='blue')
    ...
    ensure_printed(rf'다운로드 및 merge 결과 확인 시도', print_color='blue')
    try:
        ensure_printed(rf'explorer "{f_va}"', print_color='blue')
        subprocess.check_output(rf'explorer "{f_va}"', shell=True).decode('utf-8').split("\n")
    except Exception as e:
        ensure_printed(f'''{traceback.format_exc()}  {'%%%FOO%%%' if LTA else ''}''', print_color='red')

    ensure_printed(f' 불필요 리소스 삭제 시도')
    try:
