...
def ensure_command_excuted_to_os_via_powershell_exe(cmd, console_keep_mode=False, admin_mode=False):
    import time
    ...
    while 1:
        if is_front_window_title(window_title_seg=window_title_seg):
            # ensure_copied_and_pasted_with_keeping_clipboard(string=rf"cd {wsl_pnx} | xclip -sel clip", mode="wsl") # fail: cd는 xclip 으로 pipe 할 수 없음
