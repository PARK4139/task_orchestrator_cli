...
def xcopy_without_overwrite(pnx, pnx_future):
    ...
    try:
        ...
        ensure_command_excuted_to_os_like_person_as_admin(rf'echo a | xcopy "{pnx}" "{pnx_future}" /e /h /k')
        if is_f(pnx):
            ensure_command_excuted_to_os_like_person_as_admin(rf'echo f | xcopy "{pnx}" "{pnx_future}" /e /h /k')
        else:
            ensure_command_excuted_to_os_like_person_as_admin(rf'echo d | xcopy "{pnx}" "{pnx_future}" /e /h /k')
    except Exception:
