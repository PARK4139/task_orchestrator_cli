...
def ensure_docker_installed_in_wsl_via_paramiko():
    ...
    try:
        ...
        if not os.path.exists(ssh_key_path):
            ensure_printed(" SSH 키가 없습니다. SSH 키를 먼저 생성해주세요.")
            return False
        ...
        try:
            ...
            ensure_printed(" SSH 연결 성공")
        except Exception as e:
            ensure_printed(f" SSH 연결 실패: {str(e)}")
            return False
        ...
        if exit_status != 0:
            ...
            ensure_printed(f" 시스템 업데이트 실패: {error_output}")
            return False
        ...
        for cmd in docker_install_commands:
            ...
            if exit_status != 0:
                error_output = stderr.read().decode()
                ensure_printed(f" Docker 설치 실패: {error_output}")
                return False
        ...
        if exit_status == 0:
            ...
            ensure_printed(f" Docker 설치 완료: {output}")
            ssh.close()
            return True
        else:
            error_output = stderr.read().decode()
            ensure_printed(f" Docker 설치 확인 실패: {error_output}")
            ssh.close()
            return False
