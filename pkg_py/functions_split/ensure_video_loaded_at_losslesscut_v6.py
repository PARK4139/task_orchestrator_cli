...
def ensure_video_loaded_at_losslesscut_v6(max_files=30):
    ...
    from pkg_py.system_object.directories import D_DOWNLOADS, D_PKG_PKL, D_PK_WORKING
    ...
    try:
        ...
        ensure_printed(f"[DATA] len(historical_pnx_list)={len(historical_pnx_list)}")
        ...
        while True:
            ...
            if f_video_to_load is None:
                log_step("STATE= VIDEO_NULL → RELOAD")
                f_video_list_allowed = get_video_filtered_list(d_working, ext_allowed_list, video_ignored_keyword_list)[:max_files]
                f_video_to_load = get_f_video_to_load(f_video_list_allowed)
                continue

            prev_state = log_state_change(state, prev_state)
            ...
            if state == {'running': 1, 'loading': 1, 'loaded': 1, 'playing': 0} and is_window_opened("정리 중 - LosslessCut"):
                log_step("STATE= VIDEO_END_DETECTED → RESET STATE")
                state = {'running': 0, 'loading': 0, 'loaded': 0, 'playing': 0}
                continue
            ...
            if is_window_title_opened("LosslessCut"):
                log_step("STATE= WINDOW_OPENED → RELOAD FILE")
                ensure_f_video_loaded_on_losslesscut(f_video_to_load)
                state['playing'] = 0
                continue
            ...
            if state == {'running': 1, 'loading': 1, 'loaded': 1, 'playing': 0}:
                log_step("STATE= READY_TO_PLAY → PLAY")
                ...
                while True:
                    ...
                    if is_window_title_front(window_title):
                        ensure_video_playied_at_losslesscut()
                        state['playing'] = 1
                        log_step("STATE= ACTION=PLAY_STARTED")
                        break

            if state == {'running': 1, 'loading': 0, 'loaded': 1, 'playing': 0}:
                log_step("STATE= LOADED → PLAY_FULLSCREEN")
                window_title = f"{get_nx(f_video_to_load)} - LosslessCut"
                while True:
                    ensure_window_to_front(window_title_seg=window_title)
                    if is_window_title_front(window_title):
                        ...
                        ensure_pressed("f11")
                        ensure_slept(milliseconds=300)
                        log_step("STATE= ACTION=PLAY_FULLSCREEN")
                        break
                state = {'running': 1, 'loading': 1, 'loaded': 1, 'playing': 1}

            ensure_interval_based_on_state_updated(
                pk_db=pk_db,
                state_current=state,
                func_n=func_n,
