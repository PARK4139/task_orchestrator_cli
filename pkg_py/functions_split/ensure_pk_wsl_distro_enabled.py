...
def ensure_pk_wsl_distro_enabled():
    """
    ...
    ensure_printed(f"PK_WSL_DISTRO_N={PK_WSL_DISTRO_N}{' %%%FOO%%%' if LTA else ''}")
    ensure_printed(f"DEFAULT_DISTRO={DEFAULT_DISTRO}{' %%%FOO%%%' if LTA else ''}")

    wsl_distro_name_enabled = get_wsl_distro_name(cmd="wsl -l -v")
    ensure_printed(f'''wsl_distro_name_enabled={wsl_distro_name_enabled} {'%%%FOO%%%' if LTA else ''}''')
    # # 1) 이미 PK_WSL_DISTRO_N 확인
    if PK_WSL_DISTRO_N or rf'* {PK_WSL_DISTRO_N}' == wsl_distro_name_enabled:
        ensure_printed(f"'{PK_WSL_DISTRO_N}' 배포판이 이미 설치되어 있습니다. 설치를 건너뜁니다.")
        return True
    ...
    if DEFAULT_DISTRO not in wsl_distro_name_enabled:
        ...
        ensure_printed(f"'{DEFAULT_DISTRO}' 배포판이 이미 설치되어 있습니다.")
    ...
    # 4) 등록 확인 및 tar 파일 삭제
    wsl_distro_name_enabled_after = get_wsl_distro_name(cmd="wsl -l -v")
    ensure_printed(f'''wsl_distro_name_enabled_after={wsl_distro_name_enabled_after} {'%%%FOO%%%' if LTA else ''}''')
    if PK_WSL_DISTRO_N or rf'* {PK_WSL_DISTRO_N}' in wsl_distro_name_enabled_after:
        ensure_printed(f"'{PK_WSL_DISTRO_N}' 배포판이 정상적으로 import되었습니다.")
        if tar_path.exists():
            ensure_printed(f"'{tar_path}' 파일을 삭제합니다.")
            tar_path.unlink()
        return True
    else:
