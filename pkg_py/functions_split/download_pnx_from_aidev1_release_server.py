...
def download_pnx_from_aidev1_release_server(remote_f_src, local_d_dst, **config_aidev1_release_server):
    ...
    ensure_printed(str_working=rf'''src="{remote_f_src}"  {'%%%FOO%%%' if LTA else ''}''')

    f_dst = rf"{local_d_dst}/{get_nx(remote_f_src)}"
    f_dst = get_pnx_windows_style(f_dst)
    ensure_printed(f'''f_dst={f_dst} {'%%%FOO%%%' if LTA else ''}''')
    ...
    try:
        ...
        ensure_printed(f"started to download pnx {remote_f_src} to {f_dst}")
        sftp.get(remote_f_src, f_dst)
        ensure_printed(f"download pnx {f_dst}", print_color='green')

    except paramiko.AuthenticationException:
        ensure_printed("ssh Authentication failed. Please check your user_n and pw.", print_color='red')
        raise
