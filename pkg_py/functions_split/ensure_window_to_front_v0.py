        ...
        while 1:
            ...
            if is_window_title_opened(window_title=window_title):
                            ...
                            if win32gui.IsWindowVisible(hwnd):
                                ...
                                if window_title.lower() in current_window_title.lower():
                                    ...
                                    try:
                                        ...
                                        ensure_slept(seconds=0.2)
                                        ensure_printed(f''' can not move window ({window_title}) to front.''',
                                                 print_color='red')
                ...
                if is_window_title_front(window_title=window_title):
                    ensure_printed(f'''move window ({window_title}) to front via {func_n}()''', print_color='blue')
                    break
            else:
                return
    if pid is not None:
        if not str(pid).isdigit():
            # ensure_printed(f"PID 분석 결과 숫자가 아닌 것으로 판단: {pid}  {'%%%FOO%%%' if LTA else ''}", print_color='red')
            return
        pid = int(pid)
        try:
            ...
            if process.is_running() and process.status() != psutil.STATUS_ZOMBIE:
                ...
                while hwnd:
                    ...
                    # ensure_printed(f"PID {pid}에 해당하는 창 핸들을 찾을 수 없습니다.", print_color='red')
                    pass
            else:
                # ensure_printed(f"{UNDERLINE}프로세스가 exec  중이지 않거나 좀비 상태입니다.", print_color='red')
                pass
        except psutil.NoSuchProcess:
            # ensure_printed(f"{UNDERLINE}유효하지 않은 PID입니다.", print_color='red')
            pass
        except Exception as e:
            ensure_printed(f"알 수 없는 {e}", print_color='red')
            pass
