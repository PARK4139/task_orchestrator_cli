...
def ensure_d_size_stable(d_working, limit_seconds):
    ...
    ensure_printed(f'''started at {time_s} for {limit_seconds} seconds stable monitoring{'%%%FOO%%%' if LTA else ''}''')
    while 1:
        ...
        if cur_size != prev_size or cur_count != prev_count:
            if LTA:
                ensure_printed(f"d size stable = 0")
            return 0
        time_e = time.time()
        time_delta = time_e - time_s
        if limit_seconds < time_delta:
            if LTA:
                ensure_printed(f"d size stable = 1")
            ensure_printed(f'''ensured d size is stable for {limit_seconds} seconds{'%%%FOO%%%' if LTA else ''}''')
