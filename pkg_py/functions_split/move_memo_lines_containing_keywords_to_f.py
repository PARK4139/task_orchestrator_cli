...
def move_memo_lines_containing_keywords_to_f(f_from, f_to, keyword):
    ...
    try:
        ...
        ensure_printed(f"[{func_n}]{moved_count} 개의 줄 이동 fron '{f_from}' to '{f_to}'", print_color='green')

    except FileNotFoundError as e:
        ensure_printed(f"오류: f을 찾을 수 없습니다 - {e.filename}", print_color='red')
    except Exception as e:
        ensure_printed(f"예상치 못한 오류가 발생했습니다: {e}", print_color='red')
