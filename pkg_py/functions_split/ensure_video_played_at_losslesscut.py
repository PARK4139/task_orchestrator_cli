from pkg_py.system_object.directories import D_PKG_CACHE_PRIVATE, D_G_DRIVE_PK_WORKING
...
def ensure_video_played_at_losslesscut(max_files=500):
    import time
    ...
    video_name_parts_to_ignore = None
    if LTA:
        # video_name_parts_to_ignore = [get_value_completed(key_hint='video_name_parts_to_ignore=', values=['-seg', 'SEG-'])]
    ...
    options = historical_pnxs + get_list_sorted(working_list=[D_G_DRIVE_PK_WORKING, D_PK_WORKING], mode_asc=1)
    if LTA:
    ...
    # f_videos_allowed = get_video_filtered_list(d_working, ext_allowed_list, video_name_parts_to_ignore, video_ignored_regex_patterns)[:max_files] # pk_option
    f_videos_allowed = get_video_filtered_list(d_working, ext_allowed_list, video_name_parts_to_ignore, video_ignored_regex_patterns)
    ...
    try:
        ...
        ext_allowed_list = ['.mp4', '.mkv', '.avi', '.mov', '.wmv', '.flv', '.webm']
        video_name_parts_to_ignore = ['-seg', 'SEG-']
        f_videos_allowed = get_video_filtered_list(D_DOWNLOADS, ext_allowed_list, video_name_parts_to_ignore)[:max_files]
        ...
        coords_file = os.path.join(D_PKG_CACHE_PRIVATE, "losslesscut_close_coordinates.txt")
