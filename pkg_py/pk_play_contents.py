...
if __name__ == "__main__":
    try:
        ...
        working_directory_to_play_contents = pk_db.get_values(db_id=db_id)
        pk_db.save_answer(question="Select a directory to play the contents.", options=working_directory_to_play_contents, db_id=db_id)

        if not working_directory_to_play_contents or not os.path.isdir(working_directory_to_play_contents):
            print(f"[오류] 유효한 디렉토리가 선택되지 않았습니다: {working_directory_to_play_contents}")
