...
def get_values_from_fzf_routine(file_id, options, editable):
    ...
    history_file = os.path.join(base_dir, f"fzf_{file_id}.history")
...
def get_value_via_fzf_or_history(key_name, file_id, options, editable=False):
    ...
    if not options:
        ...
        return None
    decision = get_value_completed(key_hint=rf"{key_name}=", values=[PkMessages2025.VIA_FZF, PkMessages2025.VIA_HISTORICAL_FILE])
