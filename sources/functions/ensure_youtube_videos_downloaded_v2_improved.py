...
def ensure_youtube_videos_downloaded_v2_improved():
    """
    개선된 YouTube 동영상 다운로드 함수 - 체계적인 쿠키 관리 시스템과 연동
    ...
    try:
        ...
        try:
            ...
            cookie_management_success = ensure_youtube_cookies_managed()
        ...
        if not cookie_exists:
            ...
            logging.debug("자동 쿠키 관리를 위해 'ensure_youtube_cookies_managed()' 함수를 실행해주세요.")
            ...
            cookie_guide = textwrap.dedent(f"""
                ...
                또는 자동 관리:
                python -m resources.functions.ensure_youtube_cookies_managed
            """).lstrip()

            logging.debug(cookie_guide)

            # 쿠키 없이 계속 진행할지 확인
            continue_without_cookie = input("쿠키 없이 계속 진행하시겠습니까? (y/n): ").lower().strip()
            if continue_without_cookie != 'y':
                logging.debug("쿠키 파일을 준비한 후 다시 시도해주세요.")
                return False
    ...
    if is_os_windows():
        try:
            ...
            if is_f_contained_feature_str(feature_str=youtube_clip_id_stamp, d_pnx=d_pnx):
                ...
                if cookie_exists:
                    try:
                        from sources.functions.ensure_youtube_cookies_managed import backup_successful_cookies
...
def _handle_download_error_improved(url, error_msg, cookie_exists):
    ...
    if "Sign in to confirm your age" in error_msg or "age-restricted" in error_msg:
        ...
        if not cookie_exists:
            ...
            logging.debug("     python -m resources.functions.ensure_youtube_cookies_managed")
        else:
