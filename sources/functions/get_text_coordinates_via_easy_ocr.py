...
def get_text_coordinates_via_easy_ocr(string):  # 한글인식 잘 안되는 듯하다
    import logging
    from sources.functions.get_center_of_bounding_box import get_center_of_bounding_box
    from sources.functions.get_coordinates_bounding_box import get_coordinates_bounding_box
    from sources.functions.get_screenshot import get_screenshot
    ...
    coordinates_bounding_box = get_coordinates_bounding_box(image=screenshot, search_text=string)
    # print_list_as_vertical(working_list=coordinates_bounding_box, items_name="coordinates_bounding_box")
    ...
    if get_center_of_bounding_box(coordinates_bounding_box) is not None:
        ...
        # logging.debug(string = rf'''center_x="{center_x}"  {'%%%FOO%%%' if LTA else ''}''')
        # logging.debug(string = rf'''center_y="{center_y}"  {'%%%FOO%%%' if LTA else ''}''')
