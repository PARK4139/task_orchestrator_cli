...
def get_magnets_set_from_torrent_qq(search_keyword, driver=None):
    try:
        ...
        magnet_link_set = set()
        logging.debug(f'''search_keyword={search_keyword}  {'%%%FOO%%%' if LTA else ''}''')

        if search_keyword == "":
            logging.debug(f"search_keyword is blank")
        ...
        if str_negative in body:
            logging.debug(f'''cloudflare sequrity  {'%%%FOO%%%' if LTA else ''}''')
            return magnet_link_set
        ...
        logging.debug(f'''url={url:20}  url_encoded={url_encoded:20} {'%%%FOO%%%' if LTA else ''}''')
        # driver_get_url_in_browser_and_wait_loaded(url=url, driver=driver) #fail
        ...
        # logging.debug(f'''div_row_at_row={div_row_at_row}  {'%%%FOO%%%' if LTA else ''}''')
        ...
        for title, href in detail_page_link_list:
            logging.debug(f'''title={title:80s}  href={href}''')
            detail_page_link_matched_list.append((title, href))
        # ensure_iterable_log_as_vertical(item_iterable=detail_page_link_list, item_iterable_n="detail_page_link_list")
        if len(detail_page_link_matched_list) == 0:
            logging.debug(f'''detail page links not matched  {'%%%FOO%%%' if LTA else ''}''')
            return magnet_link_set
        ensure_iterable_log_as_vertical(item_iterable=detail_page_link_matched_list, item_iterable_n="link_matched_list")
        for title, href in detail_page_link_matched_list:
            # driver_get_url_as_browser_tab_via_js(driver, href)
            logging.debug(    f'''driver.title={driver.title} keywords_required={keywords_required} {'%%%FOO%%%' if LTA else ''}''')
            ...
            if string_negative in body:
                logging.debug(f'''{string_negative} in bs4_element_ResultSet ''')
                continue
            href_list = []
            title_list = []
            detail_page_link_list = []
            ...
            for txt_inner, a_class, onclick in detail_page_link_list:
                ...
                if magnet_path:
                    ...
                    logging.debug(f'''mgt_url={mgt_url}  {'%%%FOO%%%' if LTA else ''}''')
                    ...
                    for script_tag in script_tag_list:
                        logging.debug(f'''script_tag={script_tag}  {'%%%FOO%%%' if LTA else ''}''')
                        if script_tag and "magnet:?" in script_tag.text:
                            ...
                            logging.debug(f'''magnet_url_customed={magnet_url_customed:80s}  {'%%%FOO%%%' if LTA else ''}''')
                            ...
                            if mgt_url and "/magnet/" in mgt_url:
                                ...
                                logging.debug(f'''magnet_url_customed={magnet_url_customed}  {'%%%FOO%%%' if LTA else ''}''')
                                magnet_link_set.add(magnet_url_customed)
                            else:
                                logging.debug(f'''magnet_link not found''')

                else:
                    logging.debug(f'''magnet_link does not exist''')
                break
        ensure_iterable_log_as_vertical(item_iterable=magnet_link_set, item_iterable_n="magnet_link_set")
